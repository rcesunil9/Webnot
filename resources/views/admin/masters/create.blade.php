...
<div class="card card-indigo mt-1">
  <div class="card-header">
    <h3 class="card-title">Create Masters</h3>

    <a href="{{url('inMasters')}}" class="float-right"><i class="fa fa-arrow-left"></i> Back</a>
  </div>
  <!-- /.card-header -->
  <!-- form start -->
  <form action="{{url('inMasters')}}" method="POST" enctype="multipart/form-data" id="webForm">
    @csrf
    <div class="row card-body">
      <div class="col-md-12 form-group">
        ...
        <select name="createdBy" id="createdBy" class="form-control">
            ...
            <option value="{{$cb->id}}" {{old('createdBy')==$cb->id ? 'selected' : ''}}>({{$cb->email}}) / {{$cb->name}}</option>
            @endforeach
        </select>
      </div>
      ...
      <div class="col-md-3 form-group">
        <label for="userEmail">MASTER ID <sup class="text-danger">(*)</sup></label>
