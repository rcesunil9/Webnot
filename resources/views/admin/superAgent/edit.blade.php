...
<div class="card card-indigo mt-1">
    <div class="card-header">
        ...
        <a href="{{url('inSuperAgent')}}" class="float-right"><i class="fa fa-arrow-left"></i> Back</a>
    </div>
    <!-- /.card-header -->
    <!-- form start -->
    <form action="{{url('inSuperAgent/'.$user->id)}}" method="POST" enctype="multipart/form-data" id="webForm">
        @csrf
        @method('PUT')
        <div class="row card-body">
            <div class="col-md-6 form-group">
                ...
                <select name="subAdminId" id="subAdminId" class="form-control">
                    ...
                    <option value="{{$sa->id}}" {{(count($masterList)>0 && $sa->id==$masterList[0]->createdBy) ? 'selected' : ''}}>({{$sa->email}}) / {{$sa->name}}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-6 form-group">
                ...
                <select name="masterId" id="masterId" class="form-control">
                    ...
                    <option value="{{$ml->id}}" {{$ml->id==$user->createdBy ? 'selected' : ''}}>({{$ml->email}}) / {{$ml->name}}</option>
                    @endforeach
                </select>
            </div>
