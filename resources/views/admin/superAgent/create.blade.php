...
<div class="card card-indigo mt-1">
    <div class="card-header">
        <h3 class="card-title">Create Super Agent</h3>

        <a href="{{url('inSuperAgent')}}" class="float-right"><i class="fa fa-arrow-left"></i> Back</a>
    </div>
    <!-- /.card-header -->
    <!-- form start -->
    <form action="{{url('inSuperAgent')}}" method="POST" enctype="multipart/form-data" id="webForm">
        @csrf
        <div class="row card-body">
            <div class="col-md-6 form-group">
                ...
                <select name="subAdminId" id="subAdminId" class="form-control">
                    ...
                    <option value="{{$sa->id}}" {{old('subAdminId')==$sa->id ? 'selected' : ''}}>({{$sa->email}}) / {{$sa->name}}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-3 form-group">
                <label for="userEmail">SUPER AGENT ID <sup class="text-danger">(*)</sup></label>
