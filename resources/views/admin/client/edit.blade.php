...
<div class="card card-indigo mt-1">
    ...
    <!-- /.card-header -->
    <!-- form start -->
    <form action="{{url('inClient/'.$user->id)}}" method="POST" enctype="multipart/form-data" id="webForm">
        @csrf
        @method('PUT')
        <div class="row card-body">
            ...
            <div class="col-md-3 form-group">
                ...
                <select name="agentId" id="agentId" class="form-control">
                    ...
                    <option value="{{$agent->id}}" {{$agent->id==$agentInfo->id ? 'selected' : ''}}>({{$agent->email}}) / {{$agent->name}}</option>
                    @endforeach
                </select>
            </div>
